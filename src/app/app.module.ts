import {BrowserModule} from '@angular/platform-browser';
import {NgModule} from '@angular/core';

import {AppRoutingModule} from './app-routing.module';
import {AppComponent} from './app.component';
import {ScanditSdkModule} from 'scandit-sdk-angular';
import {SubModule} from './module-subfolder/sub/sub.module';

// tslint:disable-next-line:max-line-length
const LICENSE_KEY = '';
const ENGINE_LOCATION = '/assets/scandit/';

@NgModule({
  declarations: [
    AppComponent
  ],
  imports: [
    BrowserModule,
    SubModule,
    AppRoutingModule,
    ScanditSdkModule.forRoot(LICENSE_KEY, {engineLocation: ENGINE_LOCATION, preloadEngine: true, preloadBlurryRecognition: true}),
  ],
  providers: [],
  bootstrap: [AppComponent]
})
export class AppModule { }
